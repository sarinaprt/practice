...
def calender_ac():
    ...
    def update():
        ...
        if new_note!="enter your daliy rotin" :
            with open("note.txt","r",encoding="utf-8")as f:
                lin_up=f.readlines()
            other_lin=[lin for lin in lin_up if not lin.startswith(f"-{date}:")]
            with open("note.txt","w",encoding="utf-8")as f :
    ...
    def save():
        ...
        with open("note.txt","r",encoding="utf-8")as f:
            re_lin=f.readlines()
        for li in re_lin:
            if li.startswith(f"-{date}:"):
                messagebox.showwarning("warning","you have daliy rotin in this time if you want to change it click button change")
                break
        else:
            with open("note.txt","a",encoding="utf-8")as f:
                f.write(f"-{date}:{writ}")
